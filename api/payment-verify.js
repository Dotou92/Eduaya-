...
// reconfirmant directement auprès de l'API FedaPay (jamais un simple statut envoyé par le client).
...
module.exports = async function handler(req, res) {
  ...
  try {
    // 1) Récupérer la commande, EN VÉRIFIANT LE PROPRIÉTAIRE (jamais par référence seule)
    ...
    if (!order) {
      // Ne jamais confirmer si la commande n'existe pas ou n'appartient pas à l'utilisateur
    ...
    // 2) Reconfirmer directement auprès de FedaPay (jamais se fier à un statut du navigateur)
    ...
    // 3) Comparaison stricte et complète, jamais juste le statut
    const valid =
      transaction &&
...
// Traitement atomique : crédite une seule fois, jamais deux.
// Partagé avec le webhook (voir webhook.js) — même logique, deux points d'entrée.
async function processApprovedOrder(order, supaHeaders, SUPABASE_URL) {
  // Verrouillage optimiste : on ne passe `pending` -> `paid` que si elle est ENCORE `pending`.
  // La contrainte `status=eq.pending` dans le PATCH agit comme un verrou : si un autre
  // traitement (webhook) l'a déjà fait passer à `paid` entre-temps, ce PATCH ne touche 0 ligne.
  ...
  if (!Array.isArray(lockedRows) || lockedRows.length === 0) {
    // Déjà traitée par ailleurs (webhook arrivé en même temps) : rien à refaire.
    return true;
  }
...
module.exports.processApprovedOrder = processApprovedOrder;
